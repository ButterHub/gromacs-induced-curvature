...
LIGAND=${NP%%-*}
LIPID="DOPC"
INIT="${LIGAND}-ot-2nm-sym"
SYS="${LIGAND}-ot-2nm-asym"
...
g_traj -com -f pulling_${PREV}_md.gro -s dummy.tpr -n system.ndx -ox coord.svg <<INPUT
Gold
INPUT
...
# TODO - check MDP Files, and rename them nicely. Pull lowest ligand to MAX_z (upper leaflet)REFgrp is Lipid upper bilayer, pulled is Sulfonate oxygens
grompp -f pull_eq_np_init.mdp -c pulling_${PREV}_md -n system-current.ndx -p system.top -o pulling_${i}_eq.tpr
